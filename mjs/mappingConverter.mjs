...
import { ObjectTypeComposer, SchemaComposer, upperFirst, isObject } from 'graphql-compose';
...
export function convertToSourceTC(schemaComposer, mapping, typeName, opts = {}) {
  ...
  Object.keys(properties).forEach(sourceName => {
    ...
    const gqType = propertyToSourceGraphQLType(schemaComposer, properties[sourceName], `${typeName}${upperFirst(fieldName)}`, _objectSpread({}, opts, {
...
export function propertyToSourceGraphQLType(schemaComposer, prop, typeName, opts) {
...
export function inputPropertiesToGraphQLTypes(prop, fieldName, result = {
  _all: {}
}) {
  if (!prop || typeof prop.type !== 'string' && !prop.properties) {
    throw new Error('You provide incorrect Elastic property config.');
